use ac_core::{AgentInstance, Assignment, ContextRevision, CreateContextRevision, CreateTask, DomainError, Event, Id, Run, Task, TaskState};
...
impl Store {
    ...
    pub async fn create_task(&self, input: CreateTask) -> Result<Task, DomainError> {
        if input.title.trim().is_empty() {
            return Err(DomainError::InvalidInput("task title cannot be empty".into()));
        }
        let id = Uuid::new_v4();
        let now = Utc::now();
        ...
        append_event_tx(&mut tx, "human", &input.owner_actor_id, "task", id, "task.created", json!({"state": input.state, "title": input.title}), None).await?;
    ...
    pub async fn register_agent(&self, name: &str, capabilities: &[String]) -> Result<AgentInstance, DomainError> {
        if name.trim().is_empty() { return Err(DomainError::InvalidInput("agent name cannot be empty".into())); }
        let now = Utc::now();
        if let Some(row) = sqlx::query("SELECT * FROM agent_instances WHERE name=?").bind(name).fetch_optional(&self.pool).await.map_err(storage)? {
    ...
    pub async fn get_agent(&self, id: Id) -> Result<AgentInstance, DomainError> {
        let row = sqlx::query("SELECT * FROM agent_instances WHERE id=?").bind(id.to_string())
            .fetch_optional(&self.pool).await.map_err(storage)?
        ...
        Ok(AgentInstance {
            ...
            capabilities: serde_json::from_str(row.try_get::<String,_>("capabilities_json").map_err(storage)?.as_str()).map_err(storage)?,
    ...
    pub async fn list_agents(&self) -> Result<Vec<AgentInstance>, DomainError> {
        let rows=sqlx::query("SELECT * FROM agent_instances ORDER BY name").fetch_all(&self.pool).await.map_err(storage)?;
        rows.into_iter().map(|row| {
            let id=Uuid::parse_str(row.try_get::<String,_>("id").map_err(storage)?.as_str()).map_err(storage)?;
            Ok(AgentInstance {
                id,
                name: row.try_get("name").map_err(storage)?,
                status: row.try_get("status").map_err(storage)?,
    ...
    pub async fn claim_task(&self, task_id: Id, agent_id: Id, role: &str, lease_seconds: i64) -> Result<Assignment, DomainError> {
        ...
        // Make the first statement a write. This avoids SQLite's deferred
        // read->write transaction upgrade race when multiple agents claim concurrently.
        ...
        match result {
            ...
            Err(e) => {
                if e.as_database_error().and_then(|d| d.code()).as_deref() == Some("2067") {
                    return Err(DomainError::Conflict(format!("task role '{role}' already has an active assignment")));
                }
        ...
        append_event_tx(&mut tx, "agent_instance", &agent_id.to_string(), "task", task_id, "assignment.claimed",
            json!({"assignment_id":id,"role":role,"expires_at":expires}), None).await?;
        tx.commit().await.map_err(storage)?;
        Ok(Assignment { id, task_id, role: role.to_owned(), agent_instance_id: agent_id, status:"active".into(), acquired_at:now, expires_at:expires, renewed_at:now })
    }

    pub async fn task_assignments(&self, task_id: Id) -> Result<Vec<Assignment>, DomainError> {
        let rows=sqlx::query("SELECT * FROM assignments WHERE task_id=? ORDER BY acquired_at DESC")
            .bind(task_id.to_string()).fetch_all(&self.pool).await.map_err(storage)?;
    ...
    pub async fn get_assignment(&self, id: Id) -> Result<Assignment, DomainError> {
        let row=sqlx::query("SELECT * FROM assignments WHERE id=?").bind(id.to_string())
            .fetch_optional(&self.pool).await.map_err(storage)?
            .ok_or_else(|| DomainError::NotFound(format!("assignment {id}")))?;
        row_to_assignment(row)
    }

    pub async fn renew_assignment(&self, id: Id, actor_agent_id: Id, lease_seconds: i64) -> Result<Assignment, DomainError> {
        let assignment=self.get_assignment(id).await?;
        if assignment.agent_instance_id != actor_agent_id {
            return Err(DomainError::Conflict("assignment belongs to another agent instance".into()));
        }
        if assignment.status != "active" {
            return Err(DomainError::Conflict(format!("assignment is {}", assignment.status)));
        }
        let now=Utc::now();
        ...
        let expires=now + Duration::seconds(lease_seconds.max(30));
        let mut tx=self.pool.begin().await.map_err(storage)?;
        sqlx::query("UPDATE assignments SET renewed_at=?,expires_at=? WHERE id=? AND status='active'")
            .bind(now.to_rfc3339()).bind(expires.to_rfc3339()).bind(id.to_string())
            .execute(&mut *tx).await.map_err(storage)?;
        append_event_tx(&mut tx,"agent_instance",&actor_agent_id.to_string(),"task",assignment.task_id,"assignment.renewed",
            json!({"assignment_id":id,"expires_at":expires}),None).await?;
        tx.commit().await.map_err(storage)?;
    ...
    pub async fn expire_stale_assignments(&self) -> Result<u64, DomainError> {
        ...
        for row in rows {
            ...
            if result.rows_affected()==1 {
                count+=1;
                append_event_tx(&mut tx,"system","lease-manager","task",task_id,"assignment.expired",
                    json!({"assignment_id":id,"agent_instance_id":agent_id}),None).await?;
            }
        }
        tx.commit().await.map_err(storage)?;
        Ok(count)
    ...
    pub async fn start_run(&self, assignment_id: Id, actor_agent_id: Id, external_session_ref: Option<String>) -> Result<Run, DomainError> {
        let row=sqlx::query("SELECT task_id,agent_instance_id,status,expires_at FROM assignments WHERE id=?")
            .bind(assignment_id.to_string()).fetch_optional(&self.pool).await.map_err(storage)?
        ...
        if status!="active" || expires <= Utc::now() { return Err(DomainError::Conflict("assignment is not active".into())); }
        let task_id=Uuid::parse_str(row.try_get::<String,_>("task_id").map_err(storage)?.as_str()).map_err(storage)?;
        let agent_id=Uuid::parse_str(row.try_get::<String,_>("agent_instance_id").map_err(storage)?.as_str()).map_err(storage)?;
        if agent_id != actor_agent_id { return Err(DomainError::Conflict("assignment belongs to another agent instance".into())); }
        let id=Uuid::new_v4(); let now=Utc::now();
        let mut tx=self.pool.begin().await.map_err(storage)?;
        ...
        append_event_tx(&mut tx,"agent_instance",&agent_id.to_string(),"run",id,"run.started",json!({"task_id":task_id,"assignment_id":assignment_id}),None).await?;
        tx.commit().await.map_err(storage)?;
        self.get_run(id).await
    }

    pub async fn get_run(&self, id: Id) -> Result<Run, DomainError> {
        let row=sqlx::query("SELECT * FROM runs WHERE id=?").bind(id.to_string()).fetch_optional(&self.pool).await.map_err(storage)?
            .ok_or_else(|| DomainError::NotFound(format!("run {id}")))?;
        row_to_run(row)
    }

    pub async fn checkpoint_run(&self, id: Id, actor_agent_id: Id, checkpoint: Value) -> Result<Run, DomainError> {
        let run=self.get_run(id).await?;
        if run.agent_instance_id != actor_agent_id { return Err(DomainError::Conflict("run belongs to another agent instance".into())); }
        if run.status!="running" && run.status!="paused" { return Err(DomainError::Conflict(format!("run is {}",run.status))); }
        let mut tx=self.pool.begin().await.map_err(storage)?;
        sqlx::query("UPDATE runs SET checkpoint_json=? WHERE id=?").bind(checkpoint.to_string()).bind(id.to_string()).execute(&mut *tx).await.map_err(storage)?;
        append_event_tx(&mut tx,"agent_instance",&run.agent_instance_id.to_string(),"run",id,"run.checkpointed",json!({"task_id":run.task_id}),None).await?;
        tx.commit().await.map_err(storage)?;
        self.get_run(id).await
    }

    pub async fn complete_run(&self, id: Id, actor_agent_id: Id, result: Value) -> Result<Run, DomainError> {
        let run=self.get_run(id).await?;
        if run.agent_instance_id != actor_agent_id { return Err(DomainError::Conflict("run belongs to another agent instance".into())); }
        if run.status!="running" && run.status!="paused" { return Err(DomainError::Conflict(format!("run is {}",run.status))); }
        let now=Utc::now();
        ...
        let assignment_role: String = sqlx::query_scalar("SELECT role FROM assignments WHERE id=?")
            .bind(run.assignment_id.to_string()).fetch_one(&mut *tx).await.map_err(storage)?;
        ...
        append_event_tx(&mut tx,"agent_instance",&run.agent_instance_id.to_string(),"run",id,"run.completed",json!({"task_id":run.task_id,"role":assignment_role}),None).await?;
        if assignment_role == "executor" {
            sqlx::query("UPDATE tasks SET state='done', updated_at=? WHERE id=?")
                .bind(now.to_rfc3339()).bind(run.task_id.to_string()).execute(&mut *tx).await.map_err(storage)?;
            append_event_tx(&mut tx,"agent_instance",&run.agent_instance_id.to_string(),"task",run.task_id,"task.state_changed",json!({"state":"done"}),None).await?;
        }
        tx.commit().await.map_err(storage)?;
        self.get_run(id).await
    }

    pub async fn create_context_revision(&self, task_id: Id, input: CreateContextRevision) -> Result<ContextRevision, DomainError> {
        ...
        let (parent,version)=if let Some(row)=latest {
            (Some(row.try_get::<String,_>("id").map_err(storage)?), row.try_get::<i64,_>("version").map_err(storage)?+1)
        } else {(None,1)};
        ...
        sqlx::query("UPDATE tasks SET current_context_revision_id=?,updated_at=? WHERE id=?")
            .bind(id.to_string()).bind(now.to_rfc3339()).bind(task_id.to_string()).execute(&mut *tx).await.map_err(storage)?;
        let actor_type = if input.created_by_actor_id.starts_with("agent:") { "agent_instance" }
            else if input.created_by_actor_id.starts_with("system:") { "system" } else { "human" };
        append_event_tx(&mut tx,actor_type,&input.created_by_actor_id,"task",task_id,"context.revised",json!({"context_revision_id":id,"version":version}),None).await?;
        tx.commit().await.map_err(storage)?;
    ...
    pub async fn get_current_context(&self, task_id: Id) -> Result<ContextRevision, DomainError> {
        ...
        let id=task.current_context_revision_id.ok_or_else(|| DomainError::NotFound(format!("context for task {task_id}")))?;
        self.get_context_revision(id).await
    }

    pub async fn get_context_revision(&self, id: Id) -> Result<ContextRevision, DomainError> {
        let row=sqlx::query("SELECT * FROM context_revisions WHERE id=?").bind(id.to_string()).fetch_optional(&self.pool).await.map_err(storage)?
    ...
    pub async fn task_events(&self, task_id: Id) -> Result<Vec<Event>, DomainError> {
        ...
        ).bind(task_id.to_string()).bind(task_id.to_string()).fetch_all(&self.pool).await.map_err(storage)?;
...
fn row_to_task(row: sqlx::sqlite::SqliteRow) -> Result<Task, DomainError> {
    ...
    Ok(Task {
        ...
        current_context_revision_id: parse_opt_id(row.try_get("current_context_revision_id").map_err(storage)?)?,
        created_at: parse_dt(row.try_get("created_at").map_err(storage)?)?,
...
async fn append_event_tx(
    tx:&mut sqlx::Transaction<'_,sqlx::Sqlite>, actor_type:&str, actor_id:&str, entity_type:&str, entity_id:Id,
    event_type:&str, payload:Value, correlation_id:Option<&str>
) -> Result<(),DomainError> {
...
fn parse_id(v:String)->Result<Uuid,DomainError>{ Uuid::parse_str(&v).map_err(storage) }
fn parse_opt_id(v:Option<String>)->Result<Option<Uuid>,DomainError>{ v.map(|x| parse_id(x)).transpose() }
fn parse_dt(v:String)->Result<DateTime<Utc>,DomainError>{ DateTime::parse_from_rfc3339(&v).map(|x|x.with_timezone(&Utc)).map_err(storage) }
fn parse_opt_dt(v:Option<String>)->Result<Option<DateTime<Utc>>,DomainError>{ v.map(parse_dt).transpose() }
fn parse_json(v:String)->Result<Value,DomainError>{ serde_json::from_str(&v).map_err(storage) }
fn parse_opt_json(v:Option<String>)->Result<Option<Value>,DomainError>{ v.map(parse_json).transpose() }
fn storage<E:std::fmt::Display>(e:E)->DomainError{ DomainError::Storage(e.to_string()) }
