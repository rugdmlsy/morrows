use ac_core::{CreateContextRevision, CreateTask, DomainError, TaskState};
use ac_store::Store;
use serde_json::json;
...
fn test_db() -> (String, PathBuf) {
    let path = std::env::temp_dir().join(format!("agent-company-{}.db", Uuid::new_v4()));
...
async fn vertical_slice_survives_reopen_and_preserves_context_history() {
    ...
    let agent = store.register_agent("codex-test", &["code".into()]).await.unwrap();
    ...
    let c1 = store.create_context_revision(task.id, CreateContextRevision {
    ...
    let c2 = store.create_context_revision(task.id, CreateContextRevision {
    ...
    let assignment = store.claim_task(task.id, agent.id, "executor", 300).await.unwrap();
    let run = store.start_run(assignment.id, agent.id, Some("external-session-1".into())).await.unwrap();
    store.checkpoint_run(run.id, agent.id, json!({"commit":"abc123","remaining":["tests"]})).await.unwrap();
    ...
    reopened.complete_run(run.id, agent.id, json!({"ok":true})).await.unwrap();
    assert_eq!(reopened.get_task(task.id).await.unwrap().state, TaskState::Done);
...
async fn run_mutations_enforce_assignment_owner() {
    ...
    let assignment = store.claim_task(task.id, owner.id, "executor", 300).await.unwrap();

    assert!(matches!(
        store.start_run(assignment.id, intruder.id, None).await.unwrap_err(),
        DomainError::Conflict(_)
    ));

    let run = store.start_run(assignment.id, owner.id, None).await.unwrap();
    assert!(matches!(
        store.checkpoint_run(run.id, intruder.id, json!({"bad":true})).await.unwrap_err(),
        DomainError::Conflict(_)
    ));
